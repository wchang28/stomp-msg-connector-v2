...
function getProcessorOnMessageHandler(broker, process_handler_paths) {
	return (function (message) {
		...
		if (handler_def && handler_def.handler) {
			try {
				var handler = (typeof handler_def.handler === 'string' ? require(handler_def.handler) : handler_def.handler);
...
// 2. ready
// 3. error
function initialize(brokers_config) {
	var p = new progress();
	p.num_brokers = 0;
	p.num_brokers_connected = 0;
	if (!brokers_config) throw 'bad broker config';
	serverContext.config = brokers_config;
	for (var broker_name in brokers_config) {	// for each broker
		...
		if (broker_config.processors) {
			for (var processor in broker_config.processors) {	// for each processor
				...
				process_handler_paths[destination] = {handler: processor_def.handler};
		...
		var broker = new StompMsgBroker(getStompClientCreator(broker_url, protocols, tlsOptions), broker_config.brokerOptions, broker_config.loginOptions, destinations);
		...
		broker.ondisconnect = getBrokerOnDisconnectHandler(broker_name, broker_config.brokerOptions.reconnectIntervalMS);
...
module.exports.getConfig = function() {return serverContext.config;};
module.exports.getBroker = function(brokerName) {return (serverContext.brokers[brokerName] ? serverContext.brokers[brokerName] : null);};
module.exports.getTxStart = function(txName) {return (serverContext.txStarts[txName] ? serverContext.txStarts[txName] : null);};